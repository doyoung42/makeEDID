...
## 원시 코드와 디코드값, 둘 다 편집 가능하게 할 때는 이름을 갈라 쓴다

색좌표(10비트 코드 vs CIE 0-1 값)와 HDR Static Metadata 휘도(8비트 코드 vs cd/m²)
둘 다 "코드로만 입력 가능해서 불편하다"는 사용자 피드백으로 고쳤다. 패턴은 AMD
FreeSync가 이미 쓰고 있던 것을 그대로 따른다 — **디코드값 = 원래 이름
(`redX`, `maxLum`), 원시 코드 = `...Code` 접미사(`redXCode`, `maxLumCode`)**.
두 경로가 같은 바이트를 가리키므로 어느 쪽으로 입력해도 결과가 같다.

인코드는 항상 confirmed forward formula의 **대수적 역함수**로 구한다(로그·제곱근
등 — 추측이 아니라 이미 확인된 공식을 반대로 푸는 것). Min luminance처럼 형제
필드(Max 코드)가 있어야 계산되는 경우, writer는 페이로드에서 그 형제 값을
직접 읽는다 — 사용자에게 두 필드를 맞춰 입력하라고 요구하지 않는다
(Range Limits의 +255 오프셋 자동 관리와 같은 원칙).

**디코드값은 `flatten.ts`에서 문자열로 낸다**(`(code/1024).toFixed(4)`처럼) —
`base.gamma`의 "defined by DI-EXT"와 같은 관례다. `kind`는 여전히 `"number"`다.
뮤테이션 프로브·필드 감사 스크립트의 `perturb()`가 `typeof value` 기준으로
분기하므로, 숫자처럼 보이는 문자열 값은 **그 스크립트들에도 별도 케이스가
필요하다** — 안 그러면 "안전한 자동 perturbation이 없다"며 조용히 검증에서
빠진다(사유 없는 실패가 아니라 그냥 커버리지 구멍이 되므로 더 놓치기 쉽다).

## 정규식을 고쳤으면 실제로 고쳐졌는지 직접 확인한다

HDR 휘도의 `...Code` 필드를 편집 가능하게 하려고 `EDITABLE_EXT_FIELD` 정규식을
고쳤다고 **생각했는데 실제로는 옛 버전이 그대로 남아 있었다** — 스크립트 편집이
조용히 실패했거나 다른 자리를 고친 것으로 보인다. `npm run audit`이 "읽기 전용인데
사유가 없다"로 정확히 잡아냈다. 코드/설정을 고친 뒤에는 **파일을 다시 열어서
실제로 바뀌었는지 눈으로 확인**하거나, 최소한 대상 동작을 직접 실행해서 확인한다 —
"편집했다"는 것과 "편집이 반영됐다"는 것은 다른 사실이다.

## 구조 재배열은 추가/삭제와 다른 연산이다 — `moveAtPath`

블록·디스크립터·standard timing의 "순서를 바꾸고 싶다"는 요청은 그 항목이 속한
배열에서 **인접 인덱스와 자리를 맞바꾸는 것**으로 충분하다(먼 위치로 옮기는 것도
반복된 인접 스왑과 같다). `structureTargetFor`가 이미 path → "어떤 배열의 몇 번째"
를 풀어주므로, `moveAtPath`는 그 배열을 찾아 스왑하고 `guarded()`로 검증하기만
하면 된다 — add/remove처럼 배열 길이가 바뀌는 연산과 달리 스왑은 바이트 개수가
바뀔 일이 없지만, 그래도 인코더 검증은 생략하지 않는다(원칙은 예외 없이 지킨다).

**"이동 가능한가"를 UI가 매 행 매 렌더 물어본다.** 실제로 스왑해보고 되돌리는
방식(clone-and-try)은 이 빈도에서 낭비다 — `canMoveAtPath`를 별도로 두어 배열
길이 경계만 확인하는 순수 조회로 만들었다. "가능한지 알아보는 것"과 "실행하는
것"은 비용이 다르면 분리한다.
